...
$es = mysqli_query($con, $sql);
    if(!$es){
        ...
        while($mostrar=mysqli_fetch_array($es)){
            ...
            $precioan=$mostrar['precioan'];
            $precioah=$mostrar['precioah'];
            $imagen=$mostrar['imagen'];
            echo "<div class='publi'>
            <img class='animacion' src='imgprod/$imagen'>
            <br>
            <div class='ofer'>$nombre<br><h5 id='des'>Precio antes: $precioan$ <br>Precio ahora: $precioah</h5><input type='submit' value='Comprar' name='but'></div>  
        </div>";
        }
    }
?>          
